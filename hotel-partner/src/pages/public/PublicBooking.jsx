...
export default function PublicBooking() {
  ...
  if (!b.hotelId) {
    return <div className="public-container empty-state" style={{ paddingTop: 48 }}>
  ...
  if (success) {
    return (
      <div className="public-container" style={{ paddingTop: 48, paddingBottom: 48, maxWidth: 600, textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
          ...
          <button className="btn btn-secondary" onClick={() => navigate('/hotels')}>Browse More</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="public-container" style={{ paddingTop: 24, paddingBottom: 48, maxWidth: 900 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24 }}>
        <div>
          <div className="card" style={{ padding: 24, marginBottom: 16 }}>
            ...
            <div className="form-grid">
              <div className="form-group"><label className="label">Full Name</label><input className="input" value={guestName} onChange={e => setGuestName(e.target.value)} placeholder="Enter full name" /></div>
              <div className="form-group"><label className="label">Email</label><input className="input" value={guestEmail} onChange={e => setGuestEmail(e.target.value)} placeholder="Enter email" /></div>
              <div className="form-group"><label className="label">Phone</label><input className="input" value={guestPhone} onChange={e => setGuestPhone(e.target.value)} placeholder="Enter phone number" /></div>
              <div className="form-group"><label className="label">Special Requests</label><input className="input" value={specialRequests} onChange={e => setSpecialRequests(e.target.value)} placeholder="Any special requests?" /></div>
            </div>
          </div>
          <div className="card" style={{ padding: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {methods.map(m => {
                const Icon = m.icon
                return (
                  <button key={m.id} onClick={() => setMethod(m.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 16px', border: method === m.id ? '2px solid var(--primary)' : '1px solid var(--border)', borderRadius: 'var(--radius)', background: method === m.id ? 'var(--primary-light)' : 'var(--surface)', fontWeight: 600, fontSize: '0.85rem', transition: 'all 0.15s' }}>
        ...
        <div>
          <div className="public-booking-card">
            ...
            <div className="public-booking-row"><span>Check-in</span><span>{formatDate(b.checkIn)}</span></div>
            <div className="public-booking-row"><span>Check-out</span><span>{formatDate(b.checkOut)}</span></div>
            <div className="public-booking-row"><span>Nights</span><span>{b.nights}</span></div>
            <div className="public-booking-row"><span>Guests</span><span>{b.guests}</span></div>
            <div className="public-booking-row"><span>Rooms</span><span>{b.roomsCount}</span></div>
            <div className="public-booking-divider" />
            <div className="public-booking-row"><span>Base Amount</span><span>{formatPrice(b.baseAmount)}</span></div>
            {b.discount > 0 && <div className="public-booking-row" style={{ color: 'var(--success)' }}><span>Discount</span><span>-{formatPrice(b.discount)}</span></div>}
            <div className="public-booking-row"><span>Taxes</span><span>{formatPrice(b.taxAmount)}</span></div>
            <div className="public-booking-row total"><span>Total</span><span>{formatPrice(b.totalAmount)}</span></div>
            {error && <div style={{ color: 'var(--error)', fontSize: '0.85rem', marginTop: 12 }}>{error}</div>}
