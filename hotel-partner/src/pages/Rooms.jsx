...
export default function Rooms() {
  ...
  const save = (e) => {
    ...
    else setRows(prev => [...prev, { ...form, id: `r${++nextId}` }])
    setShowForm(false)
  }

  const toggleStatus = (r) => setRows(prev => prev.map(x => x.id === r.id ? { ...x, status: x.status === 'active' ? 'inactive' : 'active' } : x))
  const remove = (r) => { if (!confirm('Delete this room type?')) return; setRows(prev => prev.filter(x => x.id !== r.id)) }
                  ...
                  <tr key={r.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button className="btn btn-secondary btn-sm" style={{ fontSize: '0.75rem' }} onClick={() => toggleStatus(r)}>{r.status === 'active' ? 'Deactivate' : 'Activate'}</button>
            ...
            <div className="modal-body">
              <form onSubmit={save}>
                ...
                <div className="form-grid">
                  <div className="form-group"><label className="label">Bed Type</label><select className="input" value={form.bed_type} onChange={e => upd('bed_type', e.target.value)}><option>King</option><option>Queen</option><option>2 Queen</option><option>King + Sofa</option><option>King + 2 Twin</option><option>Twin</option></select></div>
